...
const techSkillsApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    createTechSkills: build.mutation({
      query: (data) => ({
        url: `/tech/create-skill`, // Ensure the URL is correct
        method: "POST",
        body: data, // Using 'body' for payload in RTK Query
      }),
      invalidatesTags: [tagTypes.techSkills], // This assumes 'tagTypes.experiences' is defined in tagTypes
    }),

    getAllTechSkills: build.query({
      query: (arg: Record<string, any>) => ({
        url: "/tech/skills", // Ensure this URL is correct
        method: "GET",
      ...
      providesTags: [tagTypes.techSkills], // Make sure this is correctly set
    }),
...
export const { useCreateTechSkillsMutation, useGetAllTechSkillsQuery } =
  techSkillsApi;
